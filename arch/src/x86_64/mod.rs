...
pub fn generate_common_cpuid(
    ...
    // Obtain cpuid entries that are adjusted to the specified CPU profile and the cpuid entries of
    ...
    // There should be relatively few cases where live migration can succeed between hosts from
    // different CPU vendors and making our checks account for that possibility would complicate
    // things substantially. We thus require that the host's cpu vendor matches the one used to
    // generate the CPU profile.
    ...
    // We now make the modifications according to the config parameters to each of the cpuid
    // entries declared above and then perform a compatibility check.
    ...
    if use_custom_profile {
        // Final compatibility checks to ensure that the CPUID values we return are compatible both
        // with the CPU profile and the host we are currently running on.
        let host_adjusted_to_profile = host_adjusted_to_profile
            .expect("profile-adjusted cpuid entries should exist when a custom profile is set");

        // Check that the host's cpuid is indeed compatible with the adjusted profile.
